...
| `novel` | false | Mechanism entry point for the novel writing mode: drives the workspace state machine (status / init / check / advance / rollback / export) so chapter continuity, fingerprints and next-step routing never depend on the model remembering them. |
